...
public class ActivitiesController : BaseApiController
{
    ...
    {
        return RequestResult(await Mediator.Send(new ActivityList.Query(), token));
    }

    [HttpGet("{id}")]
    public async Task<IActionResult> GetActivity(Guid id, CancellationToken token)
    {
        return RequestResult(await Mediator.Send(new Details.Query(id), token));
    }
    ...
    {
        activity.Id = id;
        return RequestResult(await Mediator.Send(new Edit.Command(activity), token));
    }
    ...
    {
        return RequestResult(await Mediator.Send(new Cancel.Command(id), token));
    }
    ...
    {
        return RequestResult(await Mediator.Send(new UpdateAttendance.Command(id), token));
    }

    [HttpGet(ActivitiesEndpoints.Categories)]
    public async Task<IActionResult> GetCategories(CancellationToken token)
    {
        return RequestResult(await Mediator.Send(new ActivityCategoryList.Query(), token));
    }

    [HttpGet(ActivitiesEndpoints.ChatPosts)]
    public async Task<IActionResult> GetChatPosts(Guid id, CancellationToken token)
    {
        return RequestResult(await Mediator.Send(new ChatPostList.Query(id), token));
    }
}
